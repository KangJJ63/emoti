...
public interface EmotionRepository extends JpaRepository<EmotionEntity, Long>{
    ...
    @Query(value = "SELECT emotion_type, COUNT(*) as count FROM emotion WHERE DATE(created_date) = DATE_ADD(CURDATE(), INTERVAL 1 DAY) GROUP BY emotion_type ORDER BY count DESC", nativeQuery = true)
    List<Object[]> getTodayEmotions();

    // 주간 우리의 기분 (7일)
@Query(value = "SELECT DAYOFWEEK(created_date) as weekday, emotion_type, COUNT(*) as count FROM emotion WHERE created_date BETWEEN (DATE_ADD(NOW(), INTERVAL 1 DAY) - INTERVAL 7 DAY) AND (DATE_ADD(NOW(), INTERVAL 1 DAY) - INTERVAL 1 DAY) AND email = :email GROUP BY DAYOFWEEK(created_date), emotion_type ORDER BY DAYOFWEEK(created_date), count DESC", nativeQuery = true)
    List<Object[]> getLastWeeklyEmotions();

    // 주별 우리의 기분
    @Query(value = "SELECT DAYOFWEEK(DATE(created_date)) as weekday, emotion_type, COUNT(*) as count FROM emotion WHERE DATE(created_date) BETWEEN (DATE_ADD(NOW(), INTERVAL 1 DAY) - INTERVAL 7 DAY) AND (DATE_ADD(NOW(), INTERVAL 1 DAY) - INTERVAL 1 DAY) GROUP BY DAYOFWEEK(DATE(created_date)), emotion_type ORDER BY DAYOFWEEK(DATE(created_date))", nativeQuery = true)
